 ...
 */
@Slf4j
public class MultiplePortTcpPassiveServer extends MediaServer {
    ...
    public void doStart() throws Exception{
        ...
        mediaPort = freePort;
        log.info("TcpPassiveServer start port {} ",freePort);
        Runnable task = ()->{
            try {
                serverSocket.setSoTimeout(1000*30);
                client = serverSocket.accept();
                log.info("TcpPassiveServer accept {}", client.getInetAddress());
                ...
                log.info("TcpPassiveServer inputStream close");
                stop();
            }catch (Exception ex){
                if("Socket closed".equals(ex.getMessage())){
                    log.info("Socket closed");
                }else {
                    log.error("TcpPassiveServer ex",ex);
                }
                stop();
            }
        };
    ...
    public void doStop() {
        try {
            if(serverSocket != null && !serverSocket.isClosed()){
                log.info("TcpPassiveServer serverSocket close ");
                serverSocket.close();
            }
        } catch (IOException e) {
        ...
        try {
            if(client != null && !client.isClosed()){
                log.info("TcpPassiveServer client close");
                client.close();
            }
        } catch (IOException e) {
            log.error("close ex",e);
        ...
        try {
            if(outputStream != null){
                log.info("TcpPassiveServer outputStream close");
                outputStream.close();
            }
        } catch (IOException e) {
            log.error("close ex",e);
