...
public class Jtt808Encoder {
    ...
    public static class Tcp extends MessageToByteEncoder<Jtt808Message>{

        @Override
        protected void encode(ChannelHandlerContext ctx, Jtt808Message message, ByteBuf out) throws Exception {
            ByteBuf byteBuf = toByteBuf(message);
            out.writeBytes(byteBuf);
        }
    }

    public static class Udp extends MessageToMessageEncoder<Jtt808Message> {

        @Override
        protected void encode(ChannelHandlerContext ctx, Jtt808Message message, List<Object> out) throws Exception {
            ByteBuf byteBuf = toByteBuf(message);
            // 获取客户端ip和端口
    ...
    private static ByteBuf toByteBuf(Jtt808Message message){
