...
public class AlarmFileDecoder {
    ...
    public static class Tcp extends ByteToMessageDecoder{
        ...
        protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
            Object message;
            // 808消息 0x7e 分隔符
            // 文件消息  0x30 0x31 0x63 0x64 分隔符
            int begin = in.getInt(0);
            if(begin == 0x30316364){
                message = alarmFileMessageData(in,ctx,null);
            }else {
                message = alarmFileMessage808(in);
            }
            out.add(message);
        }
    }
    ...
    public static class Udp extends MessageToMessageDecoder<DatagramPacket> {
        ...
        protected void decode(ChannelHandlerContext ctx, DatagramPacket in, List<Object> out) {
            ...
            // 808消息 0x7e 分隔符
            // 文件消息  0x30 0x31 0x63 0x64 分隔符
            int begin = byteBuf.getInt(0);
            if(begin == 0x30316364){
                message = alarmFileMessageData(byteBuf,null,in);
            }else {
                message = alarmFileMessage808(byteBuf);
            }
    ...
    private static Object alarmFileMessageData(ByteBuf in,ChannelHandlerContext ctx,DatagramPacket packet){
        AlarmFileMessageData message = new AlarmFileMessageData();
        message.setBegin(in.readInt());
        message.setName(in.readCharSequence(50, StandardCharsets.UTF_8).toString());
        message.setOffset(in.readInt());
        ...
        if(ctx != null){
            Attribute<AlarmFileSession> attribute = ctx.channel().attr(AttributeKey.valueOf("session"));
            clientId = attribute.get().getClientId();
        }
        if(packet != null){
            clientId = AlarmFileSessionManager.getInstance().getSessionBySocketAddress(packet.sender()).getClientId();
        }
        if(log.isDebugEnabled()){
            log.debug("<<<<< jtt808AlarmData 收到消息 clientId: {} \n message {}",clientId, message);
        }
        return message;
    }


    private static Object alarmFileMessage808(ByteBuf in){
        ...
        // 生成完整Jtt808消息
        AlarmFileMessage808 message = new AlarmFileMessage808();
        ...
        if(log.isTraceEnabled()){
            ...
            log.trace("<<<<< jtt808AlarmFile 收到消息 clientId: {} \n hexDump[{}]",clientId, message.toHexString());
        }
        return message;
    }
