 ...
 */
@Slf4j
public class LengthFieldBasedFrameDecoder extends DelimiterBasedFrameDecoder {
    ...
    private volatile boolean isFileDataDecoding;
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if(begin == 0x30316364 || isFileDataDecoding){
            if(in.readableBytes() < 4 + 50 + 4 + 4 ){
                return;
            }
            if(log.isTraceEnabled()){
                log.error("0x30316364 decode \n readableBytes {} \n begin {} \n name {} \n offset {}  \n length {} \n hex {}",
                        in.readableBytes(),
                        in.getInt(0),
                        in.getCharSequence(4, 50, StandardCharsets.UTF_8),
                        in.getInt(4 + 50),
                        in.getInt(4 + 50 + 4),
            ...
            if (decoded != null) {
                out.add(decoded);
                isFileDataDecoding = false;
            }else {
                isFileDataDecoding = true;
            }
        }else {
