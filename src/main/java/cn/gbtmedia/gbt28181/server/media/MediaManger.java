...
public class MediaManger {
    ...
    private static final ExecutorService GBT28181_MEDIA_POOL =
            Executors.newThreadPerTaskExecutor(
                    Thread.ofVirtual()
                            .name("vt-gbt28181-media-", 0)
                            .uncaughtExceptionHandler((t, e) -> log.error("gbt28181 media pool ex t {}", t, e))
                            .factory());
    ...
    @PostConstruct
    void init(){
        String mediaModel = serverConfig.getGbt28181().getMediaModel();
        log.info("gbt28181Media model {} {}", mediaModel,mediaService.getClass());
    }
    ...
    public MediaClient createClient(MediaParam mediaParam) {
        return mediaService.createClient(mediaParam);
    }

    public synchronized Integer getFreePort(){
        List<Integer> mediaPorts = new ArrayList<>();
        String[] split = ServerConfig.getInstance().getGbt28181().getMediaMultiplePort().split("-");
    ...
    public void run(Runnable task){
        GBT28181_MEDIA_POOL.execute(task);
    }
}
